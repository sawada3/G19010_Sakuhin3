...
#define IMAGE_ITEM2				TEXT(".\\IMAGE\\Item.png")				//あいてむ
...
BOOL MY_LOAD_IMAGE(VOID)
{
	...
	Item.image.handle = LoadGraph(Item.image.path);	//読み込み
...
VOID SISTEM_TEXT(int g, int Max)
{
	...
	if (ItemOK == FALSE)
	{
		DrawStringToHandle(TEXT_POSITION_X, TextPtY, "なにかが落ちている。", White, Font.handle);
	}
	if (ItemOK == TRUE)
	{
		ItemGet = TRUE;
		if (ItemNum == 0)
		{
			DrawStringToHandle(TEXT_POSITION_X, TextPtY, "【切符】", Red, Font.handle);
			DrawStringToHandle(TEXT_POSITION_X + FONT_SIZE * 4 + 5, TextPtY, "を見つけた。", White, Font.handle);
		}
	}
...
VOID BOY_TEXT(int g, int Max)
{
	...
	for (int cnt = 0; cnt < BOYFACE_DIV_NUM; cnt++)
	{
		...
		if (boy6flg == TRUE)
		{
			...
			if (g == 6)
			{
				DrawStringToHandle(TEXT_POSITION_X, TextPtY, _T("でもあなたは違う。\nあなたは生きたままここに来てしまった。\n"), White, Font.handle);
			}
			if (g >= Max - 1)
			{
...
VOID LEMON_TEXT(int g, int Max)
{
	...
	if (lem2flg == TRUE)
	{
		if (g == 0)
		{
			DrawStringToHandle(TEXT_POSITION_X, TextPtY, "……帰り道を教えてほしい？", White, Font.handle);
		}
		if (g == 1)
		{
